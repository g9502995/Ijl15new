...
//攻击不停
bool AntiRepeat(bool bEnable)
{
    if (bEnable) {
        Memory::WriteByte(0x00953700, 0xEB);
    } else {
        Memory::WriteByte(0x00953700, 0x7C);
    }
    return true;
}
...
//停止呼吸
bool stopBreathing(bool bEnable){
    if (bEnable) {
        Memory::WriteByte(0x00A12580, 0xEB);  //攻击后可立即打开帮助
    }
    return true;
}
...
KeyValuePair newKeyValuePairs[] = {
	{0, "http://wan130.com/game/132"}, //注册账号按钮超链接
	{1, "http://wan130.com/game/132"}, //修改密码按钮超链接
	{2, "http://wan130.com/game/132"}, //找回密码按钮超链接
	{3, "http://baidu.com/3"},
	{4, "http://baidu.com/4"},
	{5, "http://baidu.com/5"},
	{6, "http://baidu.com/6"},
    ...
    {4907, "游戏客户端在Windows NT系统普通用户账户权限下运行时发生错误。若要以普通用户账户权限使用HackShield提供的功能，需先创建HackShield Shadow账户。请参考「用户权限运行功能」说明。"},
    {4988, "%s : 获得了一个 "},
    {4989, "%s %s  :  %s   %s"},
    ...
    {5220, "仅在登机时才可用 %s."},
    {5223, "该道具只能装备一个。"},
    {5237, "无影：太惊险了！你这么弱小居然敢去挑战蝙蝠怪，真不敢相信！你去吸引蝙蝠怪的注意力，持续攻击它十分钟，我来趁机将它封印！"},
    {5238, "无影：太惊险了！你这么弱小居然敢去挑战蝙蝠怪，真不敢相信！你去吸引蝙蝠怪的注意力，持续攻击它5分钟，我来趁机将它封印！"},
    ...
    {5603, "背包已满"},
    {5617, "和 \r"},
	{5633, "http://wan130.com/game/132"}, //登录首页网站按钮连接
    {5639, " 金币"},
};
...
// 一击要害箭
bool FixSnipingDamageValue(bool bEnable)
{
    if (bEnable) {
        // 將 0x78E4DB 處的 JNZ (75) 修改為 JMP (EB)，跳過硬編碼傷害邏輯
        Memory::WriteByte(0x0078E4DB, 0xEB);
        Memory::WriteByte(0x00668DD6, 0xEB);
    }
    else {
        // 還原為原始的 JNZ
        Memory::WriteByte(0x0078E4DB, 0x75);
        Memory::WriteByte(0x00668DD6, 0x75);
    }
    return true;
}