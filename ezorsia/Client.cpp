...
int Client::setDamageCap = 99999999; // 物理伤害上限
int Client::setMAtkCap = 99999999; // 魔法攻击上限
int Client::setAccCap = 99999999; // 命中上限
int Client::setAvdCap = 99999999; // 回避上限
double Client::setAtkOutCap = 99999999; // 输出显示上限
bool Client::useTubi = true; // 使用Tubi功能（移除或大幅缩短技能、攻击、移动等动作的冷却时间（CD））
bool Client::bigLoginFrame = true; // 大型登录框
bool Client::SwitchChinese = true; // 切换中文模式
int Client::speedMovementCap = 200; // 移动速度上限
bool Client::noPassword = false; // 无密码模式
bool Client::debug = false; // 调试模式
bool Client::climbSpeedAuto = false; // 自动攀爬速度
float Client::climbSpeed = 4.0; // 攀爬速度
bool Client::zeroPointArrowFlag = true;  //近战不挥拳
...
bool Client::autoAssignNpc = true;
...
void Client::UpdateResolution() {
	...
	//去掉所有地图的背景，变成黑色
	//Memory::FillBytes(0x00639CB6, 0x90, 5);

	//切图不黑屏
	Memory::WriteByte(0x00776E65, 0xC3);
	Memory::FillBytes(0x00776E65 + 1, 0x90, 4);
}
...
void Client::MoreHook() {
	...
	Memory::ReplaceString(0x00AF6D1C, "女", "Female");
...
// 关闭NPC对话框的打字效果
void Client::FixChatfast() {
	unsigned char forceDisplayAll[] = { 0xBA, 0xFF, 0xFF, 0xFF, 0x7F, 0x90 };
	Memory::WriteByteArray(0x009A4468, forceDisplayAll, 6);
	Memory::PatchNop(0x009A448C, 6);
	Memory::PatchNop(0x009A4F0C, 6);
}