...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		if (reader.ParseError() == 0) {
			...
			Client::setDamageCap = reader.GetReal("optional", "setDamageCap", 99999999);
			Client::setMAtkCap = reader.GetReal("optional", "setMAtkCap", 99999999);
			Client::setAccCap = reader.GetReal("optional", "setAccCap", 99999);
			Client::setAvdCap = reader.GetReal("optional", "setAvdCap", 99999);
			Client::setAtkOutCap = reader.GetReal("optional", "setAtkOutCap", 99999999);
			//Client::useTubi = reader.GetBoolean("optional", "useTubi", false);
			Client::bigLoginFrame = reader.GetBoolean("general", "bigLoginFrame", true);
			Client::SwitchChinese = reader.GetBoolean("general", "SwitchChinese", true);
			...
			Client::zeroPointArrowFlag = reader.GetBoolean("optional", "zeroPointArrow", true);
		...
		AntiRepeat(true); //攻击不停
		stopBreathing(true); // 停止呼吸
		FixSnipingDamageValue(true); //一击要害箭
		InitCustomEquipSystem(); //自动加点按钮
		InitCustomAutoAssignHook(); //自动加点按钮
		
		Hook_OnOpenFullClientDownloadLink(true);  //弹出URL
		...
		Client::FixChatfast(); //关闭NPC对话打字效果
		...
		Client::NoPassword();
		Client::ZeroPointArrow();
		Client::MoreHook();
		BossHP::Hook();
		Client::WorldMap();
		Client::ApplyDeleteCharPatch();
		std::cout << "GetModuleFileName hook created" << std::endl;
		ijl15::CreateHook(); //NMCO::CreateHook();
		
		CreateThread(NULL, 0, SetItemInitThread, NULL, 0, NULL);
		
		PetHelper::ApplyPatches();
