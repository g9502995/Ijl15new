...
{
	// Fix for IME Input:
	// If ImGui doesn't want keyboard input, we skip passing IME messages to it.
	// This ensures the game receives them and can handle text confirmation properly.
	if (g_bImGuiInitialized) {
		ImGuiIO& io = ImGui::GetIO();
		bool isImeMsg = (msg == WM_IME_COMPOSITION || msg == WM_IME_STARTCOMPOSITION || 
						 msg == WM_IME_ENDCOMPOSITION || msg == WM_IME_NOTIFY || 
						 msg == WM_IME_SETCONTEXT || msg == WM_IME_REQUEST || msg == 0x0286 /*WM_IME_CHAR*/);
		
		if (isImeMsg && !io.WantCaptureKeyboard)
...
// externs removed as they are now in headers
	if (g_bImGuiInitialized)
	...
	if (SUCCEEDED(hr))
	{
		...
		// whereas in D3D9 it is 1.0f, so adjust it as needed
		float zero = 0.0f;
		ProxyInterface->SetRenderState(D3DRS_POINTSIZE_MIN, *(DWORD*)&zero);
		// The DEPTHBIAS value of -0.0f works differently than 0.0f
		// Some games require defaulting to -0.0f to work correctly
		float DepthBias = -0.0f;
		ProxyInterface->SetRenderState(D3DRS_DEPTHBIAS, *(DWORD*)&DepthBias);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::EndScene()
{
	if (!g_bImGuiInitialized)
	{
		...
		ProxyInterface->GetCreationParameters(&p);

		ImGui::CreateContext();
		ImGuiIO& io = ImGui::GetIO();
		io.ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard;
		io.MouseDrawCursor = false; //禁止显示鼠标指针
		...
		g_OriginalWndProc = (WNDPROC)SetWindowLongPtr(p.hFocusWindow, GWLP_WNDPROC, (LONG_PTR)ImGui_WndProc);
